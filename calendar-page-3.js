...
const calendar = {
    ...
    initiate: function () {
        // iterate through product blocks and collect data.
    ...
    data: {
        ...
        collectProducts: function () {
            ...
            calendar.view.containerHolder = elements[0].parentElement;
        ...
        fetchSchedule: async function () {
            ...
            function shoppingCartUrl() {
                ...
                const url = "https://www.parkcityssbs.com/api/commerce/shopping-cart?crumb={crumb}";
                const crumb = getCookie("crumb");
                const now = new Date();
                //return url.replace("{crumb}", crumb).replace("{timecode}", now.valueOf());
                return url.replace("{crumb}", crumb);
            }
    ...
    view: {
        ...
        addClickEvent: function () {            
            calendar.data.products.forEach(product => {
                var button = product.element.querySelector(".sqs-add-to-cart-button");
                button.addEventListener("click", function () { calendar.control.addToSchedule(this.parentElement.parentElement.parentElement) });
            });
        },
    ...
    control: {
        addToSchedule: function (element) {
            ...
            try {
                var match = calendar.data.products.filter(product => product.itemId == data.id);
...
class Product {
    constructor(date, element) {
        ...
        try {
            ...
            this.inStock = this.data.soldOut;
