...
class Product {
    constructor(element) {        
        this.element = element.parentElement.parentElement;
        try {
            var attribute = element.getAttribute("data-product");        
            this.data = JSON.parse(attribute);
			
            this.title = this.data.title;
            this.date = this.parseDate(this.data.title);

            this.itemId = this.data.id;

            this.inStock = false;
            if (this.data.qtyInStock > 0 ) {
                this.inStock = true;
            }

            this.peak = this.isPeakDate(this.title);
            //this.program = this.isProgram(this.title);
            //this.trainer = this.isTrainerDate(this.title);
        }
        catch (error) {
            console.error(error);
        }
    	...
    	this.program = false;    
    	this.paidTraining = false;
    }
            

    parseDate(title) {
        var match = title.match(/\d{1,2}\/\d{1,2}([\/\d{4}]*)/);
        if (match != null) {
    ...
    isPeakDate(title) {
        ...
        try {
            ...
            var excerpt = title.toLowerCase();
            // Product title should inclued the word "Peak"
            if (excerpt.includes("peak")) {
                // check product title against calendar settings for peak day indicators: like "PT4", "PT32", "PT18"
                // calendar settings are open to edit from the initialization.
            ...
            console.log("PEAK DATE ERROR: Product Title missing.  Display title for all products.", title);
        }
        return false;
    }
...
*/
class cartItem {
    constructor(cartRowId, itemId, sku, qty) {
...
const calendar = {
	settings: {
		...
		cartURL: "https://www.parkcityssbs.com/api/commerce/shopping-cart?crumb={crumb}",
        dates: {
            /* open and close determines what month the calendar starts and ends */
            open: new Date("11/01/2025"), 
            close: new Date("04/21/2026"),
            /* holiday */
            ...
            coreStart: new Date("12/14/2025"),
            coreEnd: new Date("04/05/2026"),

            thanksStart: new Date("11/23/2025"),
            thanksEnd: new Date("11/29/2025"),
            peak: [],
        },	
        requirements: {
            peakDays: 0,
	...
	initiate: function() {
		...
		// Collect a list of all the displayed products.
        try {
            calendar.data.collectAll();
        }
        catch (error) {
            console.error('Initiate - data.collectAll(): ${error}');
            return;
        }
		            		
		// set the target placeholder
        try {
            calendar.view.containerHolder = this.data.products[0].element.parentElement.parentElement.parentElement;
        }
        catch (error) {
            console.error('Initiate - view.containerHolder: ${error}');
        }
        
        // Summary block
        try {
            calendar.view.statistics2();
        }
        catch (error) {
            console.error('Initiate - view.statistics2: ${error}');
        }
        
        // load calendar view
        try {
            calendar.view.calendar(this.settings, this.data.products);
        }
        catch (error) {
            console.error('Initiate - view.calendar: ${error}');
        }
        
        // highlight peak dates
        try {
            calendar.view.peakDates(this.data.products);
        }
        catch (error) {
            console.error('Initiate - view.peakDates: ${error}');
        }
        
        // highlight trainerDates
        
        // highlight Out of stock
        try {
            calendar.view.outofStockDates(this.data.products, this.settings.button.full);
        }
        catch (error) {
            console.error('Initiate - view.outofStockDates: ${error}')
        }
       
        // Add to Cart Click addClickEvent
        calendar.view.addClickEvent();
    
        // Set referrer page
        calendar.control.referrer();

        // Highlight Schedule Dates
        // execute after short delay to allow cart to load
        setTimeout(function () {
            calendar.view.scheduledDates();
            calendar.control.updateSummary();

            // highlight Past Dates as Out of Stock
            calendar.view.pastDates();
        }, 1000);
	...
	rebuild: function() {
        ...
        calendar.view.calendar(calendar.settings, calendar.data.products);
	...
	data: {
		...
		collectProducts: function() {
			...
			const products = [];
			// set the containerHolder for date product elements
            calendar.view.containerHolder = elements[0].parentElement.parentElement.parentElement;
			...
			// Step 3
			// Modify each products HTML
            		
			
			// Step 4
			console.log("collectProducts complete.")
			return products;
		},
		fetchSchedule: function() {
			const inCart = new Set();
            const inCartDates = new Set();
			
			function shoppingCartUrl() {
				const crumb = getCookie("crumb");
				const now = new Date();
				return calendar.settings.cartURL.replace("{crumb}", crumb);
			}
            ...
            fetchData(shoppingCartUrl())
                .then( data => {
                    // Get the data from the current shopping cart
                    data.entries.forEach( entry => {
                        // collect the in cart sku numbers
                        inCart.add( entry.chosenVariant.sku );
                        // map those sku numbers to dates                        
                        inCart.forEach( sku => {
                            var date = skuToDate(sku);
                            if(date) {
                                // save the year, month, date part of the iso date string
                                inCartDates.add( date.toISOString().substring(0,10));
                            }
                        });

                        // filter the product list for products with matching dates.
                        var scheduledDates = this.products.filter( product => {
                            const isoDate = product.date.toISOString().substring(0,10);
                            return inCartDates.has(isoDate);
                        });

                        scheduledDates.forEach( product => {
            ...
            console.log("fetch schedule complete.")
            return inCartDates;          
		}
	},
	view: {
        ...
        calendar: function (settings, products) {
            ...
            // STEP 3
            // Add a tooltip to each product html block;
            ...
            months.forEach(function (month) {
                ...
                if (filteredProducts.length > 0) {
                    ...
                    startDate = filteredProducts[0].date;
                }
                else {
            ...
            function rewriteProduct(product) {
                ...
                // Change A link text to the Date String
                element.querySelector("a").innerText = date.toDateString();

                // Change the Excerpt to a full date string
                element.querySelector(".product-excerpt").innerText = date.getDate();

                // Add a ISO date string the the product block
                element.setAttribute("datetime", date.toISOString());

                // Add class names to the product block - enable css grid / calendar styles
                var clsList = ["grid-item", "grid-date"];
                element.classList.add(...clsList);

                // Basic button text - opend to add
                var buttonText = element.querySelector(".add-to-cart-text");
                buttonText.innerText = calendar.settings.button.off;

                // Text displayed will adding the product to cart                
                var buttonLoader = element.querySelector(".cart-loader");
                buttonLoader.innerText = calendar.settings.button.adding;
                
                // Alter the added to cart text - to our custom setting
                var buttonComplete = element.querySelector(".cart-added-text");
                buttonComplete.innerText = calendar.settings.button.on;                
            }
        ...
        monthBlock: function (startDate, filteredList) {
            ...
            // it iterates through the month and determines if a product or filler product should be inserted
            for (var day = 1; day <= daysInMonth; day++) {
                // 
                if (nextDate === undefined) {
                    var date = new Date(year, month, day);
                    var filler = fillerDateBlock(date);
                    var product = new DateProduct(date, filler);
                    set.add(product);
                }
        ...
        scheduledDates: function () {
            ...
            calendar.data.products.forEach(product => {
                ...
                if (product.element) {
                    if (product.inStock) {
                        ...
                        // Basic button text - opend to add
                        var buttonText = product.element.querySelector(".add-to-cart-text");
                        buttonText.innerText = calendar.settings.button.off;

                        // Text displayed will adding the product to cart                
                        var buttonLoader = product.element.querySelector(".cart-loader");
                        buttonLoader.innerText = calendar.settings.button.adding;
                        
                        // Alter the added to cart text - to our custom setting
                        var buttonComplete = product.element.querySelector(".cart-added-text");
                        buttonComplete.innerText = calendar.settings.button.on;                         
                    }
                }
            });
            ...
            calendar.data.products
            .filter(product => product.scheduled == true)
            .forEach(product => {
                if (product.element) {
                    ...
                    // Basic button text - opend to add
                    var buttonText = product.element.querySelector(".add-to-cart-text");
                    buttonText.innerText = calendar.settings.button.on 
                    ...
                    // Alter the added to cart text - to our custom setting
                    var buttonComplete = product.element.querySelector(".cart-added-text");
                    buttonComplete.innerText = calendar.settings.button.off                      
        ...
        outofStockDates: function (products, buttonText) {
            products.filter(product => product.inStock == false).forEach(product => {
                ...
                if (button != null) {
                    ...
                    // Basic button text - opend to add
                    var buttonText = product.element.querySelector(".add-to-cart-text");
                    buttonText.innerText = calendar.settings.button.full; 
                    ...
                    // Alter the added to cart text - to our custom setting
                    var buttonComplete = product.element.querySelector(".cart-added-text");
                    buttonComplete.innerText = calendar.settings.button.full;
        ...
        pastDates: function() {
            calendar.data.products
            .filter( product => product.date < ( new Date() ))
            //.filter( product => !product.element.classList.contains("outOfStock"))
            .forEach( product => {
                product.element.classList.add("outOfStock");
                var button = product.element.querySelector(".sqs-add-to-cart-button-inner");
                if ( button != null ) {
                      
                    // EXPIRED PRODUCTS - We have passed these dates
        ...
        addClickEvent: function () {            
            calendar.data.products.forEach(product => {
                ...
                button.addEventListener(
                    "click", 
                    function () { 
                        //calendar.control.addToSchedule(this.parentElement.parentElement.parentElement) 
                        calendar.control.addToSchedule(product.itemId)
                    }
                );
            });
        },
	...
	control: {
        ...
        addToSchedule: function(itemId) {
            var match = calendar.data.products.filter(product => product.itemId == itemId);
        ...
        referrer: function() {
            this.setCookie("calendar", window.location.pathname, 120);
        },
